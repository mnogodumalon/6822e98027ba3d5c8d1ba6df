...
const BILDER_FIELDS = [
  { key: 'bild_datei', label: 'Bilddatei', type: 'file' },
  { key: 'bild_titel', label: 'Titel des Bildes', type: 'string/text' },
  { key: 'bild_beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'bild_aufnahmedatum', label: 'Aufnahmedatum und -zeit', type: 'date/datetimeminute' },
  { key: 'bild_standort', label: 'Aufnahmeort', type: 'applookup/select', targetEntity: 'standorte', targetAppId: 'STANDORTE', displayField: 'standort_name' },
];
const STANDORTE_FIELDS = [
  { key: 'standort_name', label: 'Name des Standorts', type: 'string/text' },
  { key: 'standort_beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'standort_position', label: 'Geografische Position', type: 'geo' },
];

const ENTITY_TABS = [
  { key: 'bilder', label: 'Bilder', pascal: 'Bilder' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('bilder');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'bilder': return (data as any).bilder as Bilder[] ?? [];
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'bilder' && fieldKey === 'bild_standort') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'bilder': return BILDER_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'bilder': return {
        create: (fields: any) => LivingAppsService.createBilderEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateBilderEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteBilderEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'bilder' || dialogState?.entity === 'bilder') && (
        <BilderDialog
          open={createEntity === 'bilder' || dialogState?.entity === 'bilder'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'bilder' ? handleUpdate : (fields: any) => handleCreate('bilder', fields)}
          defaultValues={dialogState?.entity === 'bilder' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'bilder' && (
        <BilderViewDialog
          open={viewState?.entity === 'bilder'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'bilder', record: r }); }}
          standorteList={(data as any).standorte ?? []}
        />
      )}
