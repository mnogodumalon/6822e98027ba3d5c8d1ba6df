...
export function StandorteDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false, enablePhotoLocation = true }: StandorteDialogProps) {
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "standort_name": string | null, // Name des Standorts\n  "standort_beschreibung": string | null, // Beschreibung\n}`;
      ...
      if (gps) {
        setFields(f => ({ ...f, standort_position: { lat: gps.latitude, long: gps.longitude, info: geoAddr } as any }));
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="standort_name">Name des Standorts</Label>
            <Input
              id="standort_name"
          ...
          <div className="space-y-2">
            <Label htmlFor="standort_beschreibung">Beschreibung</Label>
            <Textarea
              id="standort_beschreibung"
              value={fields.standort_beschreibung ?? ''}
              onChange={e => setFields(f => ({ ...f, standort_beschreibung: e.target.value }))}
              rows={3}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="standort_position">Geografische Position</Label>
            <div className="space-y-3">
              <Button type="button" variant="outline" className="w-full" disabled={locating} onClick={() => geoLocate("standort_position")}>
                {locating ? <IconLoader2 className="h-4 w-4 mr-1.5 animate-spin" /> : <IconCrosshair className="h-4 w-4 mr-1.5" />}
                Aktuellen Standort verwenden
              </Button>
              {geoFromPhoto && fields.standort_position && (
                <p className="text-xs text-primary italic">Standort aus Foto übernommen</p>
              )}
              {fields.standort_position?.info && (
                <p className="text-sm text-muted-foreground break-words whitespace-normal">
                  {fields.standort_position.info}
                </p>
              )}
              {fields.standort_position?.lat != null && fields.standort_position?.long != null && (
                <GeoMapPicker
                  lat={fields.standort_position.lat}
                  lng={fields.standort_position.long}
                  onChange={(lat, lng) => handleMapMove("standort_position", lat, lng)}
                />
              )}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <Input type="number" step="any"
                      value={fields.standort_position?.lat ?? ''}
                      onChange={e => {
                        const v = e.target.value;
                        setFields(f => ({ ...f, standort_position: { ...(f.standort_position as any ?? {}), lat: v ? Number(v) : undefined } }));
                      }}
                    />
                  </div>
                  <div>
                    ...
                    <Input type="number" step="any"
                      value={fields.standort_position?.long ?? ''}
                      onChange={e => {
                        const v = e.target.value;
                        setFields(f => ({ ...f, standort_position: { ...(f.standort_position as any ?? {}), long: v ? Number(v) : undefined } }));
                      }}
                    />
                  </div>
                </div>
